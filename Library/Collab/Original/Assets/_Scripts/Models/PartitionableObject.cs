	...
	void OnTriggerEnter2D(Collider2D other) {
		...
		if(!isTangible && !isFilling && other.gameObject.CompareTag ("Needle")
			...
			needle.hasHit = needle.onlyHitOnce;
				...
				EventManager.ToggleLeftRightButtons(false);
			#endif
			//TODO:; CHANGE TO needle.GetSliceCount()
			Debug.Log("Needle Slice Count: " + needle.GetSliceCount());
	...
	void OnTriggerExit2D(Collider2D other) {
		...
		if(other.gameObject.CompareTag("Player")) {
			animator.SetBool("isCollidingPlayer", true);
				...
				EventManager.ToggleLeftRightButtons(true);
			#endif
		}
		DisableFractionLabel ();
...
//        if (!DataManager.DoesKeyExist(currentKey))
		if(poBoxKey == null)
        {
			...
			poBoxKey = currentKey;
			PlayerData.Instance.IncrementPOIndex();
		} else {

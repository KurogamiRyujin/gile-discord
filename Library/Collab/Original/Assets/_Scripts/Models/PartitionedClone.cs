...
//			}
		...
		while (partitions [0].gameObject.transform.position != newPosition) {
			...
			for (int i = 0; i < partitionSize; i++) {
				partitions [i].gameObject.transform.position = Vector3.MoveTowards (partitions[i].gameObject.transform.position, newPositions[i], animationSpeed*Time.deltaTime);
			}
...
//		Debug.Log ("Partitions: " + partitions.Count);
		Debug.Log("Partitioned CLone");
		if (enabled && !original.IsFilling()) {
//			playerController.canMove (false);
			Debug.Log("Partitioned CLone: Entered");
			#if UNITY_STANDALONE
			Debug.Log("Partitioned CLone: Entered Unity Standalone");

...
//				float fracJourney = distCovered / journeyLength;
				this.pointer.transform.localPosition = Vector2.MoveTowards (this.pointer.transform.localPosition, pos, POINTER_MOVEMENT_SPEED * Time.deltaTime);
...
//					(float)-0.1));
//				highlightRenderer.SetPosition (1, new Vector3(pointer.transform.position.x, highlightTransform.position.y, highlightRenderer.transform.position.z));
